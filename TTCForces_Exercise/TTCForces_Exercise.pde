...
float sepForceMaxD = 1000;
float seperationScale = 20000;
...
void setup(){
  ...
  agentPos[3] = new Vec2(50,300);
  agentPos[4] = new Vec2(100,100);
  agentPos[5] = new Vec2(850.0/2, 650.0/2);
  agentPos[6] = new Vec2(325,420);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(720,400);
  goalPos[4] = new Vec2(610,550);
  goalPos[5] = new Vec2(850.0/2, 650.0/2);
  goalPos[6] = new Vec2(720, 50);
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, agentRad*0.8*2);
  }
}
...
void moveAgents(float dt){
  ...
  for (int i = 0; i < numAgents; i++){
    if(agentPos[i].minus(goalPos[i]).length() < 0.001){ 
      agentAcc[i] = new Vec2(0,0);
      continue;
    }
    agentAcc[i] = computeAgentForces(i);
    if(agentAcc[i].length() > maxForce){agentAcc[i] = agentAcc[i].normalized().times(maxForce);}
  }
  //Update position and velocity using (Eulerian) numerical integration
  for (int i = 0; i < numAgents; i++){
    agentVel[i].add(agentAcc[i].times(dt));
    if(agentVel[i].length() > maxVelocity){agentVel[i] = agentVel[i].normalized().times(maxVelocity);}
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 avoidanceForce = getAvoidanceForce(id);
  acc.add(goal_force);
...
private Vec2 getAvoidanceForce(int i){
  Vec2 seperationForceTotal = new Vec2(0,0);
  for (int j = 0; j < numAgents; j++){ //Go through neighbors
    if(i == j) continue;
    float ttc = computeTTC(agentPos[i], agentVel[i], agentRad, agentPos[j], agentVel[j], agentRad);
    if(ttc < 0) continue;
    Vec2 aFuture = agentPos[i].plus(agentVel[i].times(ttc));
    Vec2 bFuture = agentPos[j].plus(agentVel[j].times(ttc));
    Vec2 relative_future_direction = (aFuture.minus(bFuture)).normalized();
    seperationForceTotal.add(relative_future_direction.times( k_avoid * (1/ttc)));
  }
  return seperationForceTotal;
}
