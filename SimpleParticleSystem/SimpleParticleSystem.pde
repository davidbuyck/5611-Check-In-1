...
float gravityValue = -100;
float particleLifetime = 10;

//Initalalize variable
Vec2 pos[] = new Vec2[maxParticles];
Vec2 vel[] = new Vec2[maxParticles];
float lifetime[] = new float[maxParticles];
int numBounces[] = new int[maxParticles];
int numParticles = 0;
float hyperspeedMultiplier = 3;
...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    lifetime[numParticles] = 0;
    numBounces[numParticles] = 0;
    numParticles += 1;
  }
  
  
  obstacleVel = new Vec2(0,0);
  float speedMultiplier = shiftPressed ? hyperspeedMultiplier : 1;
  
  if (leftPressed) obstacleVel = obstacleVel.plus( new Vec2(-obstacleSpeed * speedMultiplier,0));
  if (rightPressed) obstacleVel = obstacleVel.plus(new Vec2(obstacleSpeed * speedMultiplier,0));
  if (upPressed) obstacleVel = obstacleVel.plus(new Vec2(0,-obstacleSpeed * speedMultiplier));
  if (downPressed) obstacleVel = obstacleVel.plus(new Vec2(0,obstacleSpeed * speedMultiplier));
  if (obstacleVel.length() >0.1) obstacleVel.normalize();
  obstacleVel = obstacleVel.times(obstacleSpeed);
  spherePos.add(obstacleVel.times(dt));
  
  for (int i = 0; i <  numParticles; i++){
    if(lifetime[i] + dt > particleLifetime){
      pos[i] = pos[numParticles - 1];
      vel[i] = vel[numParticles - 1];
      lifetime[i] = lifetime[numParticles - 1];
      numBounces[i] = numBounces[numParticles - 1];
      numParticles--;
      continue;
    }
    lifetime[i] += dt;
    ...
    vel[i].add(new Vec2(0,-gravityValue * dt));
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      vel[i].add(obstacleVel.times(1.5));
      numBounces[i]++;
    }
  }
}
...
void draw(){
  ...
  for (int i = 0; i < numParticles; i++){
    fill(numBounces[i] * 40,255 * (lifetime[i] / particleLifetime),255 * (1 - (lifetime[i] / particleLifetime)));
