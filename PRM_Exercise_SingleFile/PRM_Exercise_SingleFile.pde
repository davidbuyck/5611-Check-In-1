...
Vec2 boxTopLeft[] = new Vec2[numBoxes];
float boxW = 100;
float boxH = 250;
...
int obsticleSelected = -1;
boolean isCircleSelected;
...
void update(float dt){
  if(obsticleSelected >= 0){
    if(isCircleSelected){
      circlePos[obsticleSelected] = new Vec2(mouseX, mouseY);
    }else{
      boxTopLeft[obsticleSelected] = new Vec2(mouseX - boxW/2, mouseY - boxH / 2);
    }
    buildPRM(circlePos, circleRad, boxTopLeft, boxW, boxH);
    runBFS(closestNode(startPos),closestNode(goalPos));
  }
...
void draw(){
  ...
  fill(250,200,200);
  for(Vec2 boxPoint : boxTopLeft){
    rect(boxPoint.x, boxPoint.y, boxW, boxH);
  }
...
int closestNode(Vec2 point){
  ...
  int closestNode = 0;
  for(int i = 0; i < numNodes; i++){
    if(nodePos[i].distanceTo(point) < closestDistance){
      closestDistance = nodePos[i].distanceTo(point);
      closestNode = i;
    }
  }
...
void mousePressed(){
  ...
  runBFS(closestNode(startPos),closestNode(goalPos));
    ...
    if(pointInBox(boxTopLeft, boxW, boxH, new Vec2(mouseX, mouseY)) >= 0){
      println("BoxClick");
      obsticleSelected = pointInBox(boxTopLeft, boxW, boxH, new Vec2(mouseX, mouseY));
      isCircleSelected = false;
      
    }
    if(pointInCircleList(circlePos,circleRad, new Vec2(mouseX, mouseY)) >= 0){
      println("CircleClick");
      obsticleSelected = pointInCircleList(circlePos,circleRad, new Vec2(mouseX, mouseY));
...
int pointInBox(Vec2 boxTopLeft[], float boxW, float boxH, Vec2 pointPos){
  for(int i = 0; i < numBoxes; i++){
    Vec2 pointToBoxPoint = pointPos.minus(boxTopLeft[i]);  
    if (pointToBoxPoint.x > 0 && pointToBoxPoint.x < boxW && pointToBoxPoint.y > 0 && pointToBoxPoint.y < boxH){ //small safety factor
      return i;
    }
  }
  return -1;
...
int pointInCircleList(Vec2[] centers, float[] radii, Vec2 pointPos){
  for (int i = 0; i < numObstacles; i++){
...
hitInfo rayBoxListIntesect(Vec2 boxTopLeft[], float boxW, float boxH, Vec2 ray_start, Vec2 ray_dir, float max_t){
  hitInfo hit = new hitInfo();
  hit.t = max_t;
  for (int i = 0; i < numBoxes; i++){
    Vec2 topLeft = boxTopLeft[i];
    
    hitInfo boxHit = rayBoxIntersect(topLeft, boxW, boxH, ray_start, ray_dir, max_t);
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft[], float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos) >= 0;
    boolean insideBox = pointInBox(boxTopLeft, boxW, boxH, randPos) >= 0; // Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos
    while (insideAnyCircle || insideBox){
      randPos = new Vec2(random(width),random(height));
      insideAnyCircle = pointInCircleList(circleCenters,circleRadii,randPos) >= 0;
      insideBox = pointInBox(boxTopLeft, boxW, boxH, randPos) >= 0;
    }
    nodePos[i] = randPos;
  }
}
  ...
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      if(distBetween < 200){
        ...
        hitInfo boxCheck =  rayBoxListIntesect(boxTopLeft, boxW, boxH, nodePos[i], dir, distBetween); //(Vec2 boxTopLeft, float boxW, float boxH, Vec2 ray_start, Vec2 ray_dir, float max_t)
